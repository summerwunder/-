...
disp('Run scenarios with varying initialisation length:')


% Run for the following weights:
Tinilist = [2,4,5,10,20];
...
% Preallocation
timey = zeros(runlen/dtc,length(Tinilist)); % for collecting computational times
errfad = zeros(4,length(Tinilist)); % for collecting set-point errors
timeyfad = zeros(4,length(Tinilist)); % for collecting average computational times
...
for includedist = [0,1]
    ...
    for segmented = [0,1] % First run is unsegmeneted, second run is segmented
        ...
        for ini = Tinilist
            ...
            disp(strcat('Tini=',num2str(ini),' run completed'))    
            runcount = runcount+1;
...
%% Plot and save
save('results/SetPointError_Varying_Tini.mat','errfad')
...
ylabel(ylab)
legend(leg1,leg2)
saveas(gcf, 'results/VaryTini_NoDisturbance.png')
...
ylabel(ylab)
legend(leg1,leg2)
saveas(gcf, 'results/VaryTini_WithDisturbance.png')
