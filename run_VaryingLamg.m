disp('Run scenarios with varying regularization weight:')


% Run for the following weights:
Lamlist = [0.05,0.1,0.2,0.5,1,2,5];
...
% Preallocation
timey = zeros(runlen/dtc,length(Lamlist)); % for collecting computational times
errfad = zeros(4,length(Lamlist)); % for collecting set-point errors
timeyfad = zeros(4,length(Lamlist)); % for collecting average computational times
...
for includedist = [0,1]
    ...
    for segmented = [0,1] % First run is unsegmeneted, second run is segmented
        ...
        for lamG = Lamlist
            ...
            disp(strcat('lambdaG=',num2str(lamG),' run completed'))    
            runcount = runcount+1;
...
%% Plot and save
save('results/SetPointError_Varying_LamG.mat','errfad')
...
ylabel(ylab)
legend(leg1,leg2)
saveas(gcf, 'results/VaryLamG_NoDisturbance.png')
...
ylabel(ylab)
legend(leg1,leg2)
saveas(gcf, 'results/VaryLamG_WithDisturbance.png')
